...
날짜: 2026-07-16
대상: `ms-timer`
선행 스펙: `2026-07-16-target-time-design.md`, `2026-07-16-i18n-toggle-design.md` (본 기능은 i18n 머지 후 그 위에서 구현한다 — 팝업 문구가 `strings.js`를 직접 쓴다)
...
**팝업 내용:** `strings.js`의 expired 문구 + 확인 버튼(`window.close()`). 테마 추종(`ms-timer:theme` localStorage를 팝업도 읽음), 레트로 대비는 카드 토큰(`--card-bg`/`--card-fg`) 사용으로 확보 — 릴 버그의 재발 방지를 위해 구현 시 대비 재계산.
