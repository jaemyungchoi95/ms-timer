...
### 스크린샷 / 테마 전환 (Task 4·5·6에서 사용) — CDP 방식

**PowerShell 전체 화면 캡처를 쓰지 말 것.** Electron 창이 포커스를 못 얻으면 그 자리에 있던 **사용자의 다른 창을 찍는다** (Task 3 실행 중 실제로 사용자의 브라우저가 찍혔다). 포커스 다툼·z-order·포커스 스틸링 방지 정책에 전부 취약하다.

**`THEMES` 배열 순서를 뒤집어 레트로로 부팅하는 트릭도 쓰지 말 것.** `readTheme()`는 localStorage를 **먼저** 읽고 저장값이 없을 때만 `THEMES[0]`으로 떨어진다. 이 환경의 Electron `userData`에는 이미 `ms-timer:theme` 값이 저장돼 있어 순서를 뒤집어도 무시된다.

대신 Chrome DevTools Protocol로 페이지를 직접 조작한다. 아래 절차는 2026-07-16에 실측 검증되었다.

**핵심 사실 — 헬퍼는 반드시 Windows node 로 실행한다.** Electron 은 Windows 프로세스라 디버깅 포트가 Windows 루프백에 바인딩된다. WSL2 는 네트워크 네임스페이스가 분리돼 있어 `127.0.0.1:9222` 가 `ECONNREFUSED` 로 거부된다. 헬퍼 파일도 Windows 경로에 둔다.

**1) 헬퍼 작성** — `/mnt/c/Users/cloocus/AppData/Local/Temp/cdp.mjs` (리포지토리 밖):
...
if (action === 'theme') {
  const r = await send('Runtime.evaluate', {
    expression: `document.documentElement.dataset.theme = ${JSON.stringify(arg)}; localStorage.setItem('ms-timer:theme', ${JSON.stringify(arg)}); document.documentElement.dataset.theme`,
    returnByValue: true,
  });
...
`WebSocket` 은 node 22+ 내장 전역이다 (WSL node v22.22.3 / Windows node v24.18.0 — 둘 다 가능). npm 패키지를 설치하지 않는다.

**2) CDP 포트로 기동** (background 로):

```bash
cd /mnt/c/Users/cloocus/ms-timer && cmd.exe /c "npx electron . --remote-debugging-port=9222"
```

**3) 테마 설정 + 캡처** — `cmd.exe` 로 Windows node 를 쓴다:

```bash
cd /mnt/c/Users/cloocus/AppData/Local/Temp && cmd.exe /c "node cdp.mjs theme retro"
cd /mnt/c/Users/cloocus/AppData/Local/Temp && cmd.exe /c "node cdp.mjs shot C:\Users\cloocus\AppData\Local\Temp\shot.png"
```

`theme` 액션은 `dataset.theme` 과 localStorage 를 **둘 다** 설정하므로 저장값이 이겨서 트릭이 무력화되는 문제가 없다. `dark` / `retro` 를 인자로 준다.

**4) 확인** — Read 툴로 `/mnt/c/Users/cloocus/AppData/Local/Temp/shot.png` 를 연다.

`Page.captureScreenshot` 은 **렌더러 페이지만** 캡처한다 — 데스크톱도, 사용자의 다른 창도 들어가지 않고, 포커스도 필요 없다.
...
캡처물과 헬퍼를 리포지토리 밖(`AppData\Local\Temp`)에 두는 이유는 커밋 오염을 막기 위해서다.

**주의:** `Page.captureScreenshot` 은 창 프레임(타이틀바)이 아니라 페이지 내용만 찍는다. 창 크기·타이틀바를 봐야 하면 사용자에게 확인을 요청한다.
...
Global Constraints의 **CDP 절차**를 따른다. 소스를 임시 편집하지 않는다 — `theme` 액션이 localStorage 를 직접 설정하므로 되돌릴 것이 없다.
...
# dark
cd /mnt/c/Users/cloocus/AppData/Local/Temp && cmd.exe /c "node cdp.mjs theme dark"
cd /mnt/c/Users/cloocus/AppData/Local/Temp && cmd.exe /c "node cdp.mjs shot C:\Users\cloocus\AppData\Local\Temp\dark.png"

# retro
cd /mnt/c/Users/cloocus/AppData/Local/Temp && cmd.exe /c "node cdp.mjs theme retro"
cd /mnt/c/Users/cloocus/AppData/Local/Temp && cmd.exe /c "node cdp.mjs shot C:\Users\cloocus\AppData\Local\Temp\retro.png"
...
**dark 에서 확인할 것:** 제목 "퇴근까지"가 플립 시계 **위**에 크게 **흰색**으로 있고, 하단 라벨이 **없고**, 시계가 아래로 내려왔다.

**retro 에서 확인할 것:** 제목이 민트 배경 위에서 **어두운 초록으로 읽힌다.** 흰색으로 보이면 `--title-fg` 가 테마별로 갈리지 않은 것이다 — 대비 1.9:1 로 Task 3 에서 고친 릴 버그와 같은 상태가 된다.

**두 테마 모두에서:** 하단에 "퇴 근 까 지" 라벨이 남아 있으면 Step 2 의 삭제가 안 된 것이다.

- [ ] **Step 8b: 테마 상태 원복**

CDP `theme` 액션이 localStorage 를 건드렸으므로 dark 로 되돌려 놓는다 (다음 태스크가 예상치 못한 테마로 뜨지 않도록).
...
Global Constraints의 **CDP 절차**로 실행하고 스크린샷을 찍는다.

**확인할 것:** 헤더 우측에 `18:00` 이 보인다. 카운트다운이 18:00 기준으로 맞다 (예: 지금이 09:40이면 `08:19:xx`).

앱은 켜둔 채로 다음 스텝으로 간다.

CDP `eval` 액션으로 저장값을 직접 확인할 수도 있다:

```bash
cd /mnt/c/Users/cloocus/AppData/Local/Temp && cmd.exe /c "node cdp.mjs eval \"localStorage.getItem('ms-timer:target')\""
```
